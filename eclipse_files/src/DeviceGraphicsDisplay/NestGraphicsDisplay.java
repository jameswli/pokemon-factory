...
public class NestGraphicsDisplay extends DeviceGraphicsDisplay {
	...
	private final ArrayList<PartGraphicsDisplay> partsInNest;
	...
	public NestGraphicsDisplay(Client c, int id) {
		...
		purgeLoc = new Location(location.getX() - Constants.PART_WIDTH,
				partStartLoc.getY());
	...
	private void animatePurge() {
		...
		if (partsInNest.size() > 0 && partsInNest.get(0) != null) {
			...
			purging = false;
			System.out.println("Invoking msgAgentPurgingDone");
			msgAgentPurgingDone();
		}
	 ...
	 * @param increment - a POSITIVE value representing number of pixels moved
	 * each call to draw
	 */
